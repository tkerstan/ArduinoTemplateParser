 ...
 * behind.
 */
int16_t TemplateParser::processTemplateUntilNextMatch(File* templateFile,
		const char* const *templateStrings, uint8_t nrOfStrings, Print* p)
{
			...
			bufferSize - maxPending)) > 0)
	{
		...
		for (i = maxPending; i < read; i++)
		{
			for (j = 0; j < nrOfStrings; j++)
			{
				...
				if (buffer[curBuffer][i]
						== pgm_read_array_byte(templateStrings,j,sPos[j]))
				{
					//Ok it does
					sPos[j] += 1;
					...
					if (pgm_read_array_byte(templateStrings,j,sPos[j]) == '\0')
					{
						...
						p->write(buffer[curBuffer],
								i
										- strlen_P(
												(char*) pgm_read_word(&templateStrings[j]))
										+ 1);
		...
		if (maxPending)
		{
			...
			memcpy(buffer[nextBuffer], &buffer[curBuffer][read - maxPending],
					maxPending);
		}
		...
		uint8_t nrOfStrings, Print* p)
{
	...
	while ((matchIdx = processTemplateUntilNextMatch(templateFile,
			templateStrings, nrOfStrings, p)) != -1)
	{
